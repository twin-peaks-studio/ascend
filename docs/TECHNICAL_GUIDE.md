...
### Phase 4.7: Entity-Linked Task Extraction (Planned)
...
#### Database: `task_entities` Junction Table
...
-- RLS: same policy pattern as other junction tables
ALTER TABLE task_entities ENABLE ROW LEVEL SECURITY;
```

**Why not reuse `entity_mentions`?** Different semantics. `entity_mentions` = "this content references entity X via #hashtag" (content-level, auto-synced from HTML). `task_entities` = "this task is about entity X" (task-level, set by AI or user). A task may link to entities that aren't mentioned in its description text.

**Why not use `initiative_entity_id`?** It's a single FK that only supports initiatives. `task_entities` supports multiple entities of any type. `initiative_entity_id` should be deprecated.

#### Extraction API Changes

**File:** `src/app/api/ai/extract-tasks/route.ts`

New request field: `entities` — array of `{ id, name, type, foundationalContext }` for entities mentioned in the source. These come from the client, which resolves them from `entity_mentions` for the source note/capture.

**File:** `src/lib/ai/prompts.ts`

Updated `UserPromptParams`:
```typescript
interface UserPromptParams {
  ...
  entities?: Array<{ id: string; name: string; type: string; foundationalContext: string | null }>;
}
```

Updated system prompt additions:
- New rule: For each extracted task, suggest which entities (by ID) it relates to
- New rule: Only link stakeholders when there's a clear dependency, follow-up, or action item involving them
- Updated output schema: `entityIds: string[]` per task

Updated user prompt: If entities are provided, append an `=== ENTITY CONTEXT ===` section listing each entity with its type, name, and foundational context (truncated if very long).
...
#### Extraction Hook Changes

**File:** `src/hooks/use-task-extraction.ts`

- `extractFromNote()` and `extractFromCapture()` accept an optional `entities` parameter
- Client resolves entities before calling the API by querying `entity_mentions` for the source note/capture, then fetching entity details
- `createSelectedTasks()` inserts `task_entities` records after task creation:
  ```typescript
  for (const entityId of task.entityIds) {
    await supabase.from("task_entities").insert({
      task_id: createdTask.id,
      entity_id: entityId,
      entity_type: entityMap.get(entityId)?.type ?? "product",
    });
  }
  ```
...
New prop: `entities?: Array<{ id: string; name: string; type: EntityType }>` — the full list of entities available for linking (scoped to those mentioned in the source).

New UI element in the meta row: Entity pills rendered after the priority selector. Each pill is colored by type (product: purple, initiative: amber, stakeholder: green) with an X button to remove. A small "+" button opens a dropdown to add entities from the available list.

Entity pills use the same color classes as entity mention pills in the rich text editor (`entity-mention--product`, `entity-mention--initiative`, `entity-mention--stakeholder` from `globals.css`).

**File:** `src/components/ai/task-extraction-dialog.tsx`

Passes `entities` prop through to `ExtractedTaskList` and `ExtractedTaskItem`.

#### Integration Points

The callers of `extractFromNote()` / `extractFromCapture()` need to resolve entities:

1. **Note detail page** (`src/app/projects/[id]/notes/[noteId]/page.tsx`): Already has access to mention data via the editor. Before calling extract, query `entity_mentions` for the note, fetch entity details (name, type, foundational_context), pass to extraction.

2. **Capture detail page** (`src/app/captures/[id]/page.tsx`): Same pattern.

#### Stakeholder Linking Rules
...
- Do NOT link stakeholders just because they were mentioned in the source — only when the task requires interaction with them

### Phase 4.8: Entity Display on Task Views (Planned)

#### Overview

All task surfaces show entity badges from `task_entities`. Entities appear as colored pills in a dedicated area of each task row/card.
...
async function fetchEntityMapForTaskIds(
  taskIds: string[]
): Promise<Map<string, TaskEntity[]>> {
  // Single query: task_entities JOIN entities
  // Returns map of task_id → TaskEntity[]
}
...
async function enrichTasksWithEntities<T extends { id: string }>(
  ...
  const entityMap = await fetchEntityMapForTaskIds(taskIds);
  return tasks.map(t => ({
    ...t,
    entities: entityMap.get(t.id) ?? [],
  }));
}
```

This replaces the current `enrichTasksWithProducts()` flow for entity display. Products derived from `project.entity_id → entity_links` are supplemented (or replaced) by direct `task_entities` records.

**Type update** (`src/types/index.ts`):
```typescript
interface TaskWithProject extends DBTask {
  project: DBProject | null;
  assignee?: DBProfile | null;
  attachments?: DBAttachment[];
  products?: TaskProduct[];     // existing (may keep for backwards compat)
  entities?: TaskEntity[];      // NEW — from task_entities
}
```
...
Add an entity pills row in the meta section. Each entity renders as a small colored pill:
- Product: `bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400`
- Initiative: `bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400`
- Stakeholder: `bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400`

Show ALL entities — no "+N more" truncation on desktop. Pills wrap naturally via `flex-wrap`.

**`TaskCard`** (`src/components/task/task-card.tsx`):

Same entity pills in the footer badges section. Replace the current single product badge with the full entity list.

**`TodayTaskRow`** (`src/app/today/page.tsx`):

Same entity pills in the badges row. Replace the current product-only badge.

#### Color Constants

Define shared entity color maps to ensure consistency across all surfaces:
...
These match the entity mention pill colors from `globals.css` (`.entity-mention--product`, etc.).

#### Migration Path from Products to Entities

The current product display (`task.products`) is derived from `project.entity_id → entity_links`. Phase 4.8 replaces this with direct `task_entities` display. The transition:

1. Existing tasks won't have `task_entities` records yet — they'll show no entity badges until re-extracted or manually linked
2. The `enrichTasksWithProducts()` function can remain as a fallback: if a task has no `task_entities` but its project has linked products, show those products with a visual indicator that they're inherited (not directly linked)
